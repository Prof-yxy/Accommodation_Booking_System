...
1. **users** (`<u>`id `</u>`, username, password, role, created_at)
2. **site_types** (`<u>`id `</u>`, name, description, image_url, base_price_overnight, base_price_daypass)
3. **campsites** (`<u>`id `</u>`, site_number, type_id, status)
   - _FK_: type_id -> site_types(id)
4. **equipments** (`<u>`id `</u>`, name, description, total_stock, price)
5. **bookings** (`<u>`id `</u>`, user_id, site_id, start_time, end_time, type, status, total_amount, created_at)
   - _FK_: user_id -> users(id), site_id -> campsites(id)
6. **booking_equipments** (`<u>`id `</u>`, booking_id, equipment_id, quantity)
